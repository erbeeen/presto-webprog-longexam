...
const ProductCard = ({ product, index }) => {
  return (
    <article className="min-h-[550px] flex flex-col rounded-3xl border-2 border-zinc-900 bg-zinc-100 p-4">
      <div className="flex aspect-4/3 items-center justify-center rounded-[1.25rem] bg-zinc-200">
        ...
        {/*   Item */}
        {/* </div> */}
        <img src={product.image} alt={product.title} className="object-contain rounded-xl" />
      </div>
      ...
      <p className="min-h-14 mt-3 text-sm leading-6 text-zinc-600">
        {product.content[0].substring(0, 120)}...
      </p>
      <Button to={`/products/${product.name}`} className="mt-auto" variant='primary'>View Product</Button>
    </article>
  );
};
